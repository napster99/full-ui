<!--
 * @Description: 防抖组件--抽象组件
 * @Author: vins
 * @Email: vins@tuya.com
 * @Date: 2019-07-23 14:40:57
 * @Lasteditors: vins
 -->
<script>
...
export default {
  ...
  props: {
    events: {
      // 格式 { event: 事件名称, im: 是否立即执行, delay: 延迟时间, handle: 回调函数 }
      type: [String, Array, Object],
      required: true
    },
    eventsOption: {
      // 格式 { im: 是否立即执行, delay: 延迟时间 }
      type: [Array, Object]
    }
  },
  methods: {
  ...
  render(h) {
    ...
    _events.forEach((item, index) => {
      ...
      if (item.im === true || curOption.im === true) {
        modifiers.im = true;
      }
      if (
      ...
      this.addDirective(vnode, 'debounce', item.event, modifiers, item.handle);
    });
    return vnode;
  }
};
